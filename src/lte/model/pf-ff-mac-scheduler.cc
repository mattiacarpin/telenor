...
PfFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
	...
	//TODO: Scheduling routine for PF scheduler

	int winnerRNTI=0;
	bool TDApproach=false;
	...
	for (int i = 0; i < rbgNum; i++)
	{
		...
		if (rbgMap.at (i) == false)
		{
			...
			//Scan each active flow to assign this RB
			for (it = m_flowStatsDl.begin (); it != m_flowStatsDl.end (); it++)
			{
				...
				if (TDApproach)
				{
					std::map<uint16_t, uint8_t>::iterator itCqiWideband = m_p10CqiRxed.find((*it).first);
					uint8_t wbCqi = 0;
					if (itCqiWideband != m_p10CqiRxed.end())
						wbCqi = (*itCqiWideband).second;
					else
						wbCqi = 1;
					if (LcActivePerFlow ((*it).first) > 0)
					{
						...
						NS_LOG_INFO (this << "TD: RNTI " << (*it).first << " MCS " << (uint32_t)mcs << " achievableRate " << achievableRate << " avgThr " << (*it).second.lastAveragedThroughput << " RCQI " << rcqi);
						//std::cout<<"TD: RNTI " << (*it).first << ", RBG "<<i<<" MCS " << (uint32_t)mcs << " achievableRate " << achievableRate<<std::endl;
						if (rcqi > rcqiMax)
						{
							rcqiMax = rcqi;
							itMax = it;
					...
					}//if LC-activePerFlow (controlla buffer non vuoto)
				}
				else
				{
					...
					if ((cqi1 > 0)||(cqi2 > 0)) // CQI == 0 means "out of range" (see table 7.2.3-1 of 36.213)
					{
						if (LcActivePerFlow ((*it).first) > 0)
						{
							...
							double rcqi = achievableRate / (*it).second.lastAveragedThroughput;
							NS_LOG_INFO (this << " RNTI " << (*it).first << "FD: MCS " << (uint32_t)mcs << " achievableRate " << achievableRate << " avgThr " << (*it).second.lastAveragedThroughput << " RCQI " << rcqi);
			...
			//std::cout<<"**** RBG "<<i<<" has been assigned to "<<winnerRNTI<<std::endl;

			std::ofstream myfile;
			myfile.open ("PFSchedulerLOG.txt",std::ios::app);
			double time=params.m_sfnSf;
			myfile<<time<<'\t';
			for (int j=0;j<10;j++)
				myfile<<0<<'\t';

			myfile<<winnerRNTI<<std::endl;
			myfile.close();
